...
def train_router(config_path: str):
    ...
    logger = setup_logging(cfg["experiment"]["output_dir"], "router")
    ...
    for epoch in range(start_epoch, cfg["training"]["num_epochs"] + 1):
        ...
        logger.info(f"[Router] Epoch {epoch} - train loss: {avg_loss:.4f}")
        ...
        logger.info(
            f"[Router] Epoch {epoch} - dev accuracy: {acc:.4f} | macro F1: {macro_f1:.4f}"
        )
