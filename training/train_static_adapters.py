...
def train_static_adapters(config_path: str):
    ...
    logger = setup_logging(cfg["experiment"]["output_dir"], "static_adapters")
    ...
    for epoch in range(start_epoch, cfg["training"]["num_epochs"] + 1):
        ...
        logger.info(f"[StaticAdapters] Epoch {epoch} - train loss: {avg_loss:.4f}")
        ...
        logger.info(
            f"[StaticAdapters] Epoch {epoch} - dev accuracy: {acc:.4f} | macro F1: {macro_f1:.4f}"
        )
        ...
        if cfg["training"]["save_every_epoch"]:
            save_checkpoint(
                ...
                epoch,
                cfg,
                experiment_name="static_adapters",
            )
...
if __name__ == "__main__":
    ...
    train_static_adapters(args.config)